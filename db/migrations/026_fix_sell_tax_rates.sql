-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- 026: Fix BUG-TAX-SQL — sell tax rates inverted + missing 10 KC minimum.
--
-- Bug introduced in migrations 023 and 024:
--   CASE WHEN v_is_cap THEN 0.05 ELSE 0.10
-- applies 5% during the group+R32 phase and 10% during KO — the opposite of
-- the intended logic (groups = 10%, KO = 5%), as defined in onlineGameStore.ts
-- and the test plan (SEC-TRADE-04).
--
-- Also missing: minimum fee of 10 KC per transaction (GREATEST(..., 10)).
-- The store applies Math.max(gross * rate, 10); the SQL had no equivalent.
--
-- Fix: replace CASE WHEN v_is_cap THEN 0.05 ELSE 0.10
--      with    GREATEST(CASE WHEN v_is_cap THEN 0.10 ELSE 0.05 END * gross, 10)
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
BEGIN
  ...
  ELSIF p_mode = 'sell' THEN
    ...
    IF NOT (p_team_id = ANY(v_eliminated)) THEN
      -- Groups + R32 (v_is_cap = true)  → 10% fee, aligned with onlineGameStore.ts
      -- KO / QF / SF / Final            →  5% fee
      -- Minimum 10 KC per transaction (same as calcTax in game-engine)
      v_fee := GREATEST(
